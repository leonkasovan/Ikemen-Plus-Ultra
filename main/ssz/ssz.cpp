...
TUserFunc(void, MemMarkBefore, Reference tag)
{
	std::wstring wtag = pu->refToWstr(tag);
	uint64_t mem = GetLiveMemory();
	g_memBeforeMap[wtag] = mem;
	std::string tagA = toNarrow(wtag);
	LOG_INFO("Memory", "[%s] BEFORE = %llu", tagA.c_str(), (unsigned long long)mem);
}

TUserFunc(void, MemMarkAfter, Reference tag)
{
	std::wstring wtag = pu->refToWstr(tag);
	uint64_t mem = GetLiveMemory();
	std::string tagA = toNarrow(wtag);
	...
	if (it != g_memBeforeMap.end()) {
		...
		LOG_INFO("Memory", "[%s] AFTER  = %llu  (WARNING: no matching BEFORE)",
			tagA.c_str(), (unsigned long long)mem);
	}
}
